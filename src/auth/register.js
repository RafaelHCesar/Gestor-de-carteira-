...
class RegisterManager {
    ...
    setupVerificationModals() {
        ...
        if (verifyEmailBtn) {
            verifyEmailBtn.addEventListener('click', () => this.selectVerificationMethod('email'));
        }

        if (verifySmsBtn) {
            verifySmsBtn.addEventListener('click', () => this.selectVerificationMethod('sms'));
        }

        if (cancelVerificationBtn) {
            cancelVerificationBtn.addEventListener('click', () => this.hideVerificationModal());
        }
    ...
    validateRegisterForm(data) {
        ...
        if (data.password) {
            if (!this.isValidPassword(data.password)) {
                errors.push('Senha deve ter pelo menos 8 caracteres, incluindo letra maiúscula, minúscula e número');
            }
    ...
    showCodeModal(method) {
        ...
        if (targetElement) {
            const target = method === 'email' ? this.userData.email : this.userData.phone;
    ...
    async sendVerificationCode(method) {
        try {
            ...
            const target = method === 'email' ? this.userData.email : this.userData.phone;
            console.log(`Código de verificação enviado para ${method} ${target}: ${this.verificationCode}`);
            
            this.showSuccess(`Código enviado para ${method === 'email' ? 'seu email' : 'seu telefone'}`);
        } catch (error) {
